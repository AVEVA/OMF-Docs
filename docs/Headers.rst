...
``messageformat``    Describes the data serialization format employed in the message body. Currently 
                     limited to ``json``.
``omfversion``       Specifies the version of the OSIsoft Message Format used in the message. The version must be of format x.x, where x must be an integer greater than or equal to 0. The version for the current specification is 1.1.
